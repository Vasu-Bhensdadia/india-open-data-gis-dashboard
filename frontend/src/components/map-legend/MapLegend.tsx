...
function CategoricalItem({ item, onHover }: { item: LegendItem; onHover?: (i: LegendItem | null) => void }) {
  return (
    <div
      role="listitem"
    ...
    >
      <div style={{ width: 16, height: 12, background: item.color, borderRadius: 2, border: '1px solid rgba(0,0,0,0.08)' }} />
      <div style={{ flex: 1 }}>{item.label}</div>
    </div>
  )
}

function GradientItem({ item, onHover }: { item: LegendItem; onHover?: (i: LegendItem | null) => void }) {
